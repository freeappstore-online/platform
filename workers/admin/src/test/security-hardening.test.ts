...
/** Replicate the admin API auth decision from index.ts for testing. */
function isAuthenticated(opts: {
  ...
  if (!opts.backendConfigured) return !opts.internalToken;
  return !!opts.bearerValid && !!opts.roles?.includes("admin");
}

describe("Security: admin auth check", () => {
  const cfg = { backendConfigured: true, internalToken: "secret-123" };

  it("allows local dev / test when backend auth is unconfigured and no internal token is set", () => {
    expect(isAuthenticated({ backendConfigured: false, bearerValid: false })).toBe(true);
  });

  it("rejects public calls without a valid FAS bearer session", () => {
    expect(isAuthenticated({ ...cfg, bearerValid: false })).toBe(false);
  });

  it("rejects signed-in non-admin users", () => {
    expect(isAuthenticated({ ...cfg, bearerValid: true, roles: ["user"] })).toBe(false);
  });

  it("allows signed-in FAS admins", () => {
    expect(isAuthenticated({ ...cfg, bearerValid: true, roles: ["user", "admin"] })).toBe(true);
  });

  it("allows service-to-service calls with the shared internal token", () => {
    // backend → /api/provision via service binding
    expect(isAuthenticated({ ...cfg, bearerValid: false, sentToken: "secret-123" })).toBe(true);
  });

  it("rejects a wrong or missing internal token without a FAS admin session", () => {
    expect(isAuthenticated({ ...cfg, bearerValid: false, sentToken: "wrong" })).toBe(false);
    expect(isAuthenticated({ ...cfg, bearerValid: false, sentToken: null })).toBe(false);
  });

  it("does not bypass when the provision token is unset on the worker", () => {
    expect(isAuthenticated({ backendConfigured: true, bearerValid: false, sentToken: "anything" })).toBe(false);
  });
});
