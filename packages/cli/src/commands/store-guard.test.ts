...
describe('assertFreeAppStoreProject — cross-store guard', () => {
  let dir: string;
  beforeEach(() => { dir = mkdtempSync(join(tmpdir(), 'store-guard-')); mkdirSync(join(dir, 'web'), { recursive: true }); });
  afterEach(() => rmSync(dir, { recursive: true, force: true }));

  it('allows a real FreeAppStore app (uses @freeappstore/sdk)', async () => {
    writeFileSync(join(dir, 'web', 'package.json'), JSON.stringify({ dependencies: { '@freeappstore/sdk': '^0.14.0' } }));
  ...
  it('rejects a FreeGameStore game by its SDK dependency (the dodge-drop case)', async () => {
    writeFileSync(join(dir, 'web', 'package.json'), JSON.stringify({ dependencies: { '@freegamestore/games': '^0.16.0' } }));
  ...
  it('rejects a project whose deploy.yml targets the fgs-games bucket', async () => {
    ...
    writeFileSync(join(dir, '.github', 'workflows', 'deploy.yml'), 'aws s3 sync ./web/dist "s3://fgs-games/games/foo/"');
    const err = await assertFreeAppStoreProject(dir);
    expect(err).toContain('fgs publish');
  });
