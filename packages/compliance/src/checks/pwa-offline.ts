 ...
 *
 *   1. Platform mandate — every app on freeappstore.online MUST be an
...
export async function checkPwaOffline(source: FileSource): Promise<CheckResult> {
  ...
  // Platform mandate (freeappstore.online): every app must be an
  ...
  if (html !== null && !hasServiceWorker) {
    return {
      ...
      status: 'fail',
      detail: 'platform mandate: every app on freeappstore.online must be an installable PWA, but no service worker is registered',
      suggestions: [
