...
const detect = (files: Record<string, string>) =>
  isGameProject(mapFileSource(new Map(Object.entries(files))));

describe('isGameProject', () => {
  it('detects @freeappstore/games in package.json', async () => {
    expect(
      await detect({ 'package.json': '{"dependencies":{"@freeappstore/games":"^0.1"}}' }),
    ).toBe(true);
  });

  it('detects an import from @freeappstore/games', async () => {
    expect(
      await detect({ 'web/src/App.tsx': 'import { GameShell } from "@freeappstore/games";' }),
    ).toBe(true);
  });
  ...
  it('detects type-only and multi-name GameShell imports', async () => {
    expect(
      await detect({ 'a.ts': "import type { GameShellProps, GameShell } from '../shell';" }),
    ).toBe(true);
