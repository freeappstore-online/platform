...
beforeEach(() => {
  globalThis.fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ value: 0 }), { status: 200 }));
});
...
describe("Counters.list", () => {
  it("fetches all counters without auth", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ likes: 5, views: 100 }), { status: 200 }));
...
describe("Counters.get", () => {
  it("returns the counter value", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ value: 42 }), { status: 200 }));
    const counters = new Counters("poll", "https://api.example", fakeAuth(null));
...
describe("Counters.increment", () => {
  it("sends POST with auth and returns new value", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({ value: 6 }), { status: 200 }));
