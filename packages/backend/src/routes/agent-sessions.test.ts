...
function fakeDB(opts: {
  user?: { id: string; github_login: string; avatar_url: string | null; date_of_birth: string | null } | null;
  ...
  const prepare = (sql: string) => {
    ...
    let bound: unknown[] = [];
    return {
      bind: (...args: unknown[]) => { bound = args; return { first: first as any, all, run }; },
      first: first as any,
      all,
      run,
    };
    async function first<T>() {
      ...
      if (trimmed.includes('FROM agent_sessions') && trimmed.includes('session_id = ?')) return (opts.session ?? null) as T;
      return null as T;
    }
    async function all() {
    ...
    async function run() { return { meta: { changes: 1 } }; }
  };
...
describe('agent-sessions', () => {
  ...
  it('GET /v1/agent/sessions returns sessions list', async () => {
    const sessions = [
      { session_id: 's1', name: 'App 1', app_id: null, app_url: null, deployed: 0, created_at: 1000, updated_at: 2000 },
    ];
    const db = fakeDB({ user, sessions });
    const res = await app.request('/v1/agent/sessions', { headers: { Authorization: await authHeader() } }, env(db));
  ...
  it('GET /v1/agent/sessions/:id returns session with messages', async () => {
    const session = {
      session_id: 's1', user_id: 'user-1', name: 'Test', app_id: null, app_url: null,
      deployed: 0, messages: '[{"role":"user","content":"hello"}]', deploy_state: null,
      created_at: 1000, updated_at: 2000,
    };
    const db = fakeDB({ user, session });
    const res = await app.request('/v1/agent/sessions/s1', { headers: { Authorization: await authHeader() } }, env(db));
    expect(res.status).toBe(200);
  ...
  it('GET /v1/agent/sessions/:id returns null for nonexistent', async () => {
    ...
    const res = await app.request('/v1/agent/sessions/nope', { headers: { Authorization: await authHeader() } }, env(db));
    expect(res.status).toBe(200);
  ...
  it('PUT /v1/agent/sessions/:id creates session', async () => {
    ...
    const res = await app.request('/v1/agent/sessions/s2', {
      method: 'PUT',
  ...
  it('PUT /v1/agent/sessions/:id/messages updates messages', async () => {
    const db = fakeDB({ user });
    const res = await app.request('/v1/agent/sessions/s1/messages', {
      method: 'PUT',
      headers: { Authorization: await authHeader(), 'Content-Type': 'application/json' },
  ...
  it('DELETE /v1/agent/sessions/:id deletes session', async () => {
    const db = fakeDB({ user });
    const res = await app.request('/v1/agent/sessions/s1', {
      method: 'DELETE',
