    ...
    prepare: (sql: string) => {
      ...
      const result = {
        first: async () => {
          ...
          if (trimmed.includes('SELECT 1 FROM counters'))
            return opts.counterExists ? { 1: 1 } : null;
          return null;
        },
        all: async () => {
          if (trimmed.includes('FROM counters')) {
            const entries = Object.entries(opts.counters ?? {}).map(([key, value]) => ({
              key,
              value,
            }));
...
describe('counter routes', () => {
  ...
  it('GET /v1/apps/:appId/counters lists counters (no auth required)', async () => {
    const res = await app.request(
      '/v1/apps/timer/counters',
      {},
      env(fakeDB({ counters: { views: 42 } })),
    );
  ...
  it('GET /v1/apps/:appId/counters/:key returns single counter', async () => {
    const res = await app.request(
      '/v1/apps/timer/counters/views',
      {},
      env(fakeDB({ counters: { views: 10 } })),
    );
    expect(res.status).toBe(200);
  ...
  it('GET /v1/apps/:appId/counters/:key returns 0 for missing counter', async () => {
    const res = await app.request(
      '/v1/apps/timer/counters/missing',
      {},
      env(fakeDB({ counters: {} })),
    );
    expect(res.status).toBe(200);
  ...
  it('POST /v1/apps/:appId/counters/:key rejects too-long key', async () => {
    ...
    const res = await app.request(
      `/v1/apps/timer/counters/${longKey}`,
      {
        method: 'POST',
        headers: { Authorization: await authHeader(), 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
