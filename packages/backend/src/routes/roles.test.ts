      ...
      const result = {
        first: async () => {
          ...
          if (trimmed.includes('FROM apps'))
            return opts.appCreator != null ? { owner_login: opts.appCreator } : null;
          if (trimmed.includes('SELECT 1 FROM app_roles'))
            return opts.existingRole ? { 1: 1 } : null;
          return null;
        },
        all: async () => {
...
describe('roles routes', () => {
  ...
  it('GET /v1/apps/:appId/roles/check/:role checks DB', async () => {
    const res = await app.request(
      '/v1/apps/timer/roles/check/editor',
      {
        headers: { Authorization: await authHeader() },
      },
  ...
  it('POST /v1/apps/:appId/roles/ensure-member creates member role', async () => {
    const res = await app.request(
      '/v1/apps/timer/roles/ensure-member',
      {
        method: 'POST',
        headers: { Authorization: await authHeader() },
      },
