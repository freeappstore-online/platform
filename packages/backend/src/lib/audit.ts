 ...
 * GitHub repo (#19). A tracked artifact is a `fail` like any other check, and
 * counts toward `failed`. The check itself degrades to `warn` only when it
 * can't see the tree (rate limit, truncated tree, repo not found).
 *
